<template>
    <div class="bg-white py-24 sm:py-32 rounded-md bg-opacity-5 bg-clip-padding backdrop-filter backdrop-blur-sm">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="mx-auto max-w-2xl lg:mx-0">
                <h2 class="text-3xl font-bold tracking-tight text-gray-400 sm:text-4xl">From the blog of afnan-shakeel</h2>
            ...
            <div class="flex justify-end mt-8">
                <button @click="toggleAddModal(true)" type="button"
                    class="rounded-md ring-black ring-1  bg-white bg-opacity-70 px-4 py-1.5 text-sm font-medium text-black hover:bg-black hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
                    Write a New Post</button>
            </div>
            <div
                class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-200 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                <article v-for="post in blogs" :key="post.id" class="flex max-w-xl flex-col items-start justify-between  border-b ">
                    ...
                    <div class="flex items-center gap-x-4 text-xs">
                        ...
                        <a :href="'#'"
                            class="relative z-10 rounded-full bg-black px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100 hover:text-black">{{
                                post.topic_tags }}</a>
                    </div>
                    <div class="group relative" @click="toggleBlog(post, true)">
                        <h3 class="mt-3 text-lg font-semibold leading-6 text-gray-400 group-hover:text-gray-600">
                            <a :href="'#'">
